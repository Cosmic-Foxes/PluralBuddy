...
export class AlertView extends TranslatedView {
	successViewCustom(string: string) {
		return [
			new Container()
				.setComponents(
					new TextDisplay().setContent(`  ${emojis.check}   ${string}`),
				)
				.setColor("#4cc270"),
		];
	}
			...
			new Container()
				.setComponents(
					new TextDisplay().setContent(
						`  ${emojis.check}   ${this.translations[translationKey]}`,
					),
				)
				.setColor("#4cc270"),
		];
	...
	errorViewCustom(string: string) {
		return [
			new Container()
				.setComponents(
					new TextDisplay().setContent(`  ${emojis.x}   ${string}`),
				)
				.setColor("#B70000"),
		];
	}
	...
	errorView(translationKey: keyof DefaultLocale) {
		return [
			new Container()
				.setComponents(
					new TextDisplay().setContent(
						`  ${emojis.x}   ${this.translations[translationKey]}`,
					),
				)
				.setColor("#B70000"),
		];
	...
	questionView(translationKey: keyof DefaultLocale) {
		return [
			new Container()
				.setComponents(
					new TextDisplay().setContent(
						`  ${emojis.circleQuestion}   ${this.translations[translationKey]}`,
					),
				)
				.setColor("#1190FF"),
		];
