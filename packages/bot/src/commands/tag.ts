...
const options = {
	"tag-name": createStringOption({
		description:
			"Name of the tag to query. You can use `<user-id>/<tag>` for tags from other systems.",
		required: true,
	}),
	...
	override async run(ctx: CommandContext<typeof options>) {
		...
		if (userTagMatch) {
			...
			query = tagCollection.findOne({
				$or: [{ tagFriendlyName: otherAlterName }, { tagId: otherAlterName }],
			...
			query = Number.isNaN(Number.parseInt(tagFriendlyName))
				? tagCollection.findOne({
						$or: [{ tagFriendlyName: tagFriendlyName }],
						systemId,
					})
				: tagCollection.findOne({
						$or: [
							{ tagFriendlyName: tagFriendlyName },
							{ tagId: tagFriendlyName },
						],
						systemId,
					});
		}
		...
		return await ctx.ephemeral(
			{
				components: [
					...new TagView(await ctx.userTranslations()).tagProfileView(
						tag,
						tag.systemId !== ctx.author.id,
					),
					...(tag.systemId === ctx.author.id
						? new TagView(await ctx.userTranslations()).tagConfigureButton(tag)
						: []),
				],
				flags:
					MessageFlags.IsComponentsV2 +
