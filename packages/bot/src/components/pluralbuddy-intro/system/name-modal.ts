...
export default class NameInteractionFormCNS extends ModalCommand {
	override filter(context: ModalContext) {
		return InteractionIdentifier.Setup.FormSelection.NameForm.startsWith(
			context.customId,
		);
	}
	...
	async run(ctx: ModalContext) {
		const rootInteractionId =
			InteractionIdentifier.Setup.FormSelection.NameForm.substring(
				ctx.customId,
			)[0] ?? "";
		...
		temporarySystem.systemName = ctx.interaction.getInputValue(
