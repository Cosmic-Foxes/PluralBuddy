...
export default class NameCNS extends ComponentCommand {
	...
	override filter(ctx: ComponentContext<typeof this.componentType>) {
		return InteractionIdentifier.Setup.CreateNewSystem.Privacy.startsWith(
			ctx.customId,
		);
	}
	...
	async run(ctx: ComponentContext<typeof this.componentType>) {
		const oldInteractionId =
			InteractionIdentifier.Setup.CreateNewSystem.Privacy.substring(
				ctx.customId,
			);
		const form = new Modal()
			.setCustomId(
				InteractionIdentifier.Setup.FormSelection.PrivacyForm.create(
					oldInteractionId[0] ?? "",
				),
			)
				...
				new Label()
					...
					.setComponent(
						new StringSelectMenu()
							...
							.setOptions(privacySelection(await ctx.userTranslations()))
							.setValuesLength({
								min: 0,
								max: privacySelection(await ctx.userTranslations()).length,
							})
							.setRequired(false),
					),
			]);
