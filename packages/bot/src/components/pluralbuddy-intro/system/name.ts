...
export default class NameCNS extends ComponentCommand {
	...
	override filter(ctx: ComponentContext<typeof this.componentType>) {
		return InteractionIdentifier.Setup.CreateNewSystem.Name.startsWith(
			ctx.customId,
		);
	}
	...
	async run(ctx: ComponentContext<typeof this.componentType>) {
		const oldInteractionId =
			InteractionIdentifier.Setup.CreateNewSystem.Name.substring(ctx.customId);
		const form = new Modal()
			.setCustomId(
				InteractionIdentifier.Setup.FormSelection.NameForm.create(
					oldInteractionId[0] as string,
				),
			)
				...
				new Label()
					...
					.setComponent(
						new TextInput()
							...
							.setCustomId(
								InteractionIdentifier.Setup.FormSelection.NameType.create(),
							)
							.setLength({ max: 20 })
							.setRequired(true),
					),
