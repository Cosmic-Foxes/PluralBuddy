...
export default class SetName extends ComponentCommand {
	...
	override filter(ctx: ComponentContext<typeof this.componentType>) {
		return InteractionIdentifier.Systems.Configuration.GeneralTab.SetNicknameFormat.equals(
			ctx.customId,
		);
	}
	...
	async run(ctx: ComponentContext<typeof this.componentType>) {
		...
		const form = new Modal()
			.setCustomId(
				InteractionIdentifier.Systems.Configuration.FormSelection.NicknameForm.create(),
			)
			...
			.addComponents([
				new Label()
					...
					.setDescription(
						(await ctx.userTranslations()).SYSTEM_NICKNAME_FORM_DESC,
					)
					.setComponent(
						new TextInput()
