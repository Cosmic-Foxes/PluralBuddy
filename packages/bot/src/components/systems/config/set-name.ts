...
export default class SetName extends ComponentCommand {
	...
	override filter(ctx: ComponentContext<typeof this.componentType>) {
		return InteractionIdentifier.Systems.Configuration.GeneralTab.SetName.equals(
			ctx.customId,
		);
	}
	...
	async run(ctx: ComponentContext<typeof this.componentType>) {
		...
		const form = new Modal()
			.setCustomId(
				InteractionIdentifier.Systems.Configuration.FormSelection.NameForm.create(),
			)
					...
					.setComponent(
						oldSystemName.length > 3
							? new TextInput()
									...
									.setCustomId(
										InteractionIdentifier.Systems.Configuration.FormSelection.NameType.create(),
									)
									.setLength({ max: 100 })
									.setRequired(true)
