...
export default class SetAlterColorButton extends ComponentCommand {
	...
	override filter(context: ComponentContext<typeof this.componentType>) {
		return InteractionIdentifier.Systems.Configuration.Alters.SetAlterColor.startsWith(
			context.customId,
		);
	}
	...
	override async run(ctx: ComponentContext<typeof this.componentType>) {
		const alterId =
			InteractionIdentifier.Systems.Configuration.Alters.SetAlterColor.substring(
				ctx.customId,
			)[0];
		...
		const form = new Modal()
			.setCustomId(
				InteractionIdentifier.Systems.Configuration.FormSelection.Alters.AlterColorForm.create(
					alter.alterId,
				),
			)
