...
export default class SetUsernameButton extends ComponentCommand {
	...
	override filter(context: ComponentContext<typeof this.componentType>) {
		return InteractionIdentifier.Systems.Configuration.Alters.SetServerDisplayName.startsWith(
			context.customId,
		);
	}
	...
	override async run(ctx: ComponentContext<typeof this.componentType>) {
		const alterId =
			InteractionIdentifier.Systems.Configuration.Alters.SetServerDisplayName.substring(
				ctx.customId,
			)[0];
		...
		const form = new Modal()
			.setCustomId(
				InteractionIdentifier.Systems.Configuration.FormSelection.Alters.AlterServerDisplayNameForm.create(
					alter.alterId,
				),
			)
