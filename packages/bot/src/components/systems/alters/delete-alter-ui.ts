...
export default class DeleteAlterButton extends ComponentCommand {
	...
	override filter(context: ComponentContext<typeof this.componentType>) {
		return InteractionIdentifier.Systems.Configuration.Alters.DeleteAlter.startsWith(
			context.customId,
		);
	}
		...
		return await ctx.update({
			components: [
				...new AlertView(await ctx.userTranslations()).questionViewCustom(
					(await ctx.userTranslations()).CONFIRMATION_ALTER_DELETION.replace(
						"%alter%",
						alter.username,
					),
				),
				...
				new ActionRow().setComponents(
					...
					new Button()
						...
						.setCustomId(
							InteractionIdentifier.Systems.Configuration.Alters.RemoveAlterConfirm.create(
								alter.alterId,
							),
						),
				),
