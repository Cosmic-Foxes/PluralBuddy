...
export default class UndoOperationButton extends ComponentCommand {
	...
	async run(ctx: ComponentContext<typeof this.componentType>) {
		...
		const operationId = InteractionIdentifier.Systems.UndoOperation.substring(
			ctx.customId,
		)[0];
		...
		await followup.edit({
			components: new AlertView(await ctx.userTranslations()).successViewCustom(
				(await ctx.userTranslations()).OPERATION_UNDO_SUCCESS.replace(
					"%value-count%",
					operation.changedOperationStrings.length.toString(),
				),
			),
		});
