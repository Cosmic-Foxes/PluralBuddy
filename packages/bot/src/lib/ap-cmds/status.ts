...
export async function runStatusCommand(ctx: CommandContext, help: boolean) {
        ...
        return ctx.ephemeral(
            {
                components: [
                    new Container().setComponents(
                        new TextDisplay().setContent(`  ${emojis.circleQuestion}   ${translations.NO_STATUS_AP}`),
    ...
    if (system.disabledGuilds.includes(guild.id)) {
        ...
        return ctx.ephemeral({
            components: [
                new Container().setComponents(
                    new TextDisplay().setContent(`  ${emojis.circleQuestion}   ${translations.DISABLED_SERVER}`),
