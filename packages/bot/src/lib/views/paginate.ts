...
export default async function paginateComponents<
	K extends { $?: I } & any,
	I extends string | undefined,
>(
	list: Partial<
		Record<
			ExtractKeys<typeof InteractionIdentifier>,
			(
				data: K & { $translations: DefaultLocale },
			) => Promise<TopLevelBuilders[]>
		>
	>,
	...
	const getFullId = (key: string | undefined) => {
		...
		return result as
			| InteractionObj<($: string | undefined) => string>
			| undefined;
	};
	...
	if (correspondingFunction === undefined)
		return [
			new TextDisplay().setContent(
				"-# Hm. There was an error while loading this pagination component. ",
			),
		];

	return [
		...((await correspondingFunction[1]({
			...data,
			$translations: lang,
		} as K & { $translations: DefaultLocale })) ?? []),
		new Container().setComponents(
			new ActionRow().setComponents(
				new Button()
					.setCustomId(
						getFullId(orderedList[page - 2]?.[0])?.create(data.$) ??
							"disabled1",
					)
				...
				new Button()
					.setCustomId(
						getFullId(orderedList[page]?.[0])?.create(data.$) ?? "disabled2",
					)
