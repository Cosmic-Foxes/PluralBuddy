...
export async function language<T extends boolean | undefined>(
	id: string,
	noCache?: T,
): Promise<T extends true ? null | string : string> {
	try {
		let data = langMemoryCache[id] ?? (await client.cache.i18n.get(id))?.l;

		if (data === undefined) {
			if (noCache) return null as T extends true ? null | string : string;
